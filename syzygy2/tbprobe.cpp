...
int MapKK[10][squareNumber]; // [MapA1D1D4][squareNumber]

int Binomial[6][squareNumber];    // [k][n] k elements from a set of n elements
int LeadPawnIdx[6][squareNumber]; // [leadPawnsCnt][squareNumber]
...
class TBTables {
    ...
    typedef std::tuple<tKey, TBTable<WDL>*, TBTable<DTZ>*> Entry;
    ...
    void insert(tKey key, TBTable<WDL>* wdl, TBTable<DTZ>* dtz) {
        uint32_t homeBucket = (uint32_t)key & (Size - 1);
        Entry entry = std::make_tuple(key, wdl, dtz);

        // Ensure last element is empty to avoid overflow when looking up
        for (uint32_t bucket = homeBucket; bucket < Size + Overflow - 1; ++bucket) {
            tKey otherKey = std::get<KEY>(hashTable[bucket]);
...
Ret do_probe_table(const Position& pos, T* entry, WDLScore wdl, ProbeState* result) {
    ...
    bool blackStronger = (pos.getMaterialKey().getKey() != entry->key);
