...
PolyglotKey::PolyglotKey():_pieceMapping{0,11,9,7,5,3,1,0,0,10,8,6,4,2,0,0}, _PG{{
...
uint64_t PolyglotKey::get(const Position& pos)
{
	uint64_t k = 0;
	...
	while( b )
	{
		...
		k ^= _PG.Zobrist.psq[_pieceMapping[p]][s];
	}
	...
	while(b)
	{
		k ^= _PG.Zobrist.castle[iterateBit(b)];
	}

	if( pos.hasEpSquare() )
	{
		k ^= _PG.Zobrist.enpassant[getFileOf(pos.getEpSquare())];
	}
